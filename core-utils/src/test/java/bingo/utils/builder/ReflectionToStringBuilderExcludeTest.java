...
/**
 * @version $Id: ReflectionToStringBuilderExcludeTest.java 1185702 2011-10-18 14:53:59Z ggregory $
 */
public class ReflectionToStringBuilderExcludeTest {
    ...
    public void test_toStringExcludeEmptyArray() {
        String toString = ReflectionToStringBuilder.toStringExclude(new TestFixture(), Arrays.EMPTY_STRING_ARRAY);
        this.validateSecretFieldPresent(toString);
    }
    ...
    private void validateNonSecretField(String toString) {
        Assert.assertTrue(toString.indexOf(NOT_SECRET_FIELD) > Arrays.INDEX_NOT_FOUND);
        Assert.assertTrue(toString.indexOf(NOT_SECRET_VALUE) > Arrays.INDEX_NOT_FOUND);
    }

    private void validateSecretFieldAbsent(String toString) {
        Assert.assertEquals(Arrays.INDEX_NOT_FOUND, toString.indexOf(SECRET_VALUE));
