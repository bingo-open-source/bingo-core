...
import bingo.utils.Arrays;

/**
 * Assists in implementing {@link java.lang.Comparable#compareTo(Object)} methods.
 * 
 * It is consistent with <code>equals(Object)</code> and <code>hashcode()</code> built with {@link EqualsBuilder} and
 * {@link HashCodeBuilder}.</p>
 * 
 * <p>
 * Two Objects that compare equal using <code>equals(Object)</code> should normally also compare equal using
 * <code>compareTo(Object)</code>.
 * </p>
 * 
 * <p>
 * All relevant fields should be included in the calculation of the comparison. Derived fields may be ignored. The same
 * fields, in the same order, should be used in both <code>compareTo(Object)</code> and <code>equals(Object)</code>.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * Alternatively, there are {@link #reflectionCompare(Object, Object) reflectionCompare} methods that use reflection to
 * determine the fields to append. Because fields can be private, <code>reflectionCompare</code> uses
 * {@link java.lang.reflect.AccessibleObject#setAccessible(boolean)} to bypass normal access control checks. This will
 * fail under a security manager, unless the appropriate permissions are set up correctly. It is also slower than
...
public class CompareToBuilder implements Builder<Integer> {
	 ...
	 * 
	 * <p>
	 * Starts off assuming that the objects are equal. Multiple calls are then made to the various append methods,
	 * followed by a call to {@link #toComparison} to get the result.
	 * </p>
	 */
	 ...
	 * 
	 * <p>
	 * Fields can be private, thus <code>AccessibleObject.setAccessible</code> is used to bypass normal access control
	 * checks. This will fail under a security manager unless the appropriate permissions are set.
	 * </p>
	 * 
	 * <ul>
	 ...
	 * <ul>
	 * <li>Static fields will not be compared</li>
	 * <li>If <code>compareTransients</code> is <code>true</code>, compares transient members. Otherwise ignores them,
	 * as they are likely derived fields.</li>
	 * <li>Superclass fields will be compared</li>
	 * </ul>
	 * 
	 * <p>
	 ...
	 * <ul>
	 * <li>Static fields will not be compared</li>
	 * <li>If the <code>compareTransients</code> is <code>true</code>, compares transient members. Otherwise ignores
	 * them, as they are likely derived fields.</li>
	...
	/**
	 * <p>
	 * Appends to <code>builder</code> the comparison of <code>lhs</code> to <code>rhs</code> using the fields defined
	 * in <code>clazz</code>.
	 * </p>
	 * 
	 * @param lhs left-hand object
	...
	private static void reflectionAppend(Object lhs, Object rhs, Class<?> clazz, CompareToBuilder builder, boolean useTransients,
		...
		for (int i = 0; i < fields.length && builder.comparison == 0; i++) {
			Field f = fields[i];
			if (!Arrays.contains(excludeFields, f.getName()) && (f.getName().indexOf('$') == -1)
	 ...
	 * <li>Check if either <code>lhs</code> or <code>rhs</code> is <code>null</code>, a <code>null</code> object is less
	 * than a non-<code>null</code> object</li>
	 ...
	 * 
	 * <p>
	 * If <code>lhs</code> is an array, array comparison methods will be used. Otherwise <code>comparator</code> will be
	 * used to compare the objects. If <code>comparator</code> is <code>null</code>, <code>lhs</code> must implement
	 * {@link Comparable} instead.
	 * </p>
	 * 
	 * @param lhs left-hand object
	 * @param rhs right-hand object
	 * @param comparator <code>Comparator</code> used to compare the objects, <code>null</code> means treat lhs as
	 ...
	 * @param lhs left-hand array
	 * @param rhs right-hand array
	 * @param comparator <code>Comparator</code> to use to compare the array elements, <code>null</code> means to treat
	 *            <code>lhs</code> elements as <code>Comparable</code>.
	 * @return this - used to chain append calls
	 * @throws ClassCastException if <code>rhs</code> is not assignment-compatible with <code>lhs</code>
	 * @since 2.0
	 */
	...
	//-----------------------------------------------------------------------
	/**
	 * Returns a negative integer, a positive integer, or zero as the <code>builder</code> has judged the "left-hand"
	 * side as less than, greater than, or equal to the "right-hand" side.
	 * 
