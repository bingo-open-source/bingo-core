 ...
 * 
 * <p>
 * The following is the approach taken. When appending a data field, the current total is multiplied by the multiplier
 * then a relevant value for that data type is added. For example, if the current hashCode is 17, and the multiplier is
 * 37, then appending the integer 45 will create a hashcode of 674, namely 17 * 37 + 45.
 * </p>
 * 
 * <p>
...
public class HashCodeBuilder implements Builder<Integer> {
	 ...
	 * 
	 * Now HashSet uses equals() to determine if two elements with the same hashcode really are equal, so we also need
	 * to ensure that the replacement objects are only equal if the original objects are identical.
	 * 
	 * The original implementation (2.4 and before) used the System.indentityHashCode() method - however this is not
	...
	private static void reflectionAppend(Object object, Class<?> clazz, HashCodeBuilder builder, boolean useTransients, String[] excludeFields) {
		...
		try {
			...
			for (Field field : fields) {
				if (!Arrays.contains(excludeFields, field.getName()) && (field.getName().indexOf('$') == -1)
	 ...
	 * @since 2.0
	 */
	public static <T> int reflectionHashCode(int initialNonZeroOddNumber, int multiplierNonZeroOddNumber, T object, boolean testTransients,
	 ...
	 * </p>
	 * <p>
	 * This is in contrast to the standard <code>java.lang.Boolean.hashCode</code> handling, which computes a
	 * <code>hashCode</code> value of <code>1231</code> for <code>java.lang.Boolean</code> instances that represent
	 * <code>true</code> or <code>1237</code> for <code>java.lang.Boolean</code> instances that represent
	 * <code>false</code>.
	 * </p>
	 * <p>
	...
	/**
	 * <p>
	 * The computed <code>hashCode</code> from toHashCode() is returned due to the likelihood of bugs in mis-calling
	 * toHashCode() and the unlikeliness of it mattering what the hashCode for HashCodeBuilder itself is.
	 * </p>
	 * 
	 * @return <code>hashCode</code> based on the fields appended
	 * @since 2.5
