...
abstract class VfsUtils extends Reflects {
	...
	static {
		...
		try {
			...
			VFS_METHOD_GET_ROOT_URL = Reflects.findMethod(vfsClass, methodName, URL.class);
			VFS_METHOD_GET_ROOT_URI = Reflects.findMethod(vfsClass, methodName, URI.class);

			Class<?> virtualFile = loader.loadClass(pkg + "VirtualFile");

			VIRTUAL_FILE_METHOD_EXISTS = Reflects.findMethod(virtualFile, "exists");
			VIRTUAL_FILE_METHOD_GET_INPUT_STREAM = Reflects.findMethod(virtualFile, "openStream");
			VIRTUAL_FILE_METHOD_GET_SIZE = Reflects.findMethod(virtualFile, "getSize");
			VIRTUAL_FILE_METHOD_GET_LAST_MODIFIED = Reflects.findMethod(virtualFile, "getLastModified");
			VIRTUAL_FILE_METHOD_TO_URI = Reflects.findMethod(virtualFile, "toURI");
			VIRTUAL_FILE_METHOD_TO_URL = Reflects.findMethod(virtualFile, "toURL");
			VIRTUAL_FILE_METHOD_GET_NAME = Reflects.findMethod(virtualFile, "getName");
			VIRTUAL_FILE_METHOD_GET_PATH_NAME = Reflects.findMethod(virtualFile, "getPathName");
			GET_PHYSICAL_FILE = Reflects.findMethod(virtualFile, "getPhysicalFile");

			methodName = (VFS_VER.V3.equals(version) ? "getChild" : "findChild");

			VIRTUAL_FILE_METHOD_GET_CHILD = Reflects.findMethod(virtualFile, methodName, String.class);

			Class<?> utilsClass = loader.loadClass(pkg + "VFSUtils");

			VFS_UTILS_METHOD_GET_COMPATIBLE_URI = Reflects.findMethod(utilsClass, "getCompatibleURI",
					virtualFile);
			VFS_UTILS_METHOD_IS_NESTED_FILE = Reflects.findMethod(utilsClass, "isNestedFile", virtualFile);

			VIRTUAL_FILE_VISITOR_INTERFACE = loader.loadClass(pkg + "VirtualFileVisitor");
			VIRTUAL_FILE_METHOD_VISIT = Reflects.findMethod(virtualFile, "visit", VIRTUAL_FILE_VISITOR_INTERFACE);

			Class<?> visitorAttributesClass = loader.loadClass(pkg + "VisitorAttributes");
			VISITOR_ATTRIBUTES_FIELD_RECURSE = Reflects.findField(visitorAttributesClass, "RECURSE");
		}
		catch (ClassNotFoundException ex) {
	...
	protected static Object invokeVfsMethod(Method method, Object target, Object... args) throws IOException {
		try {
			...
			Reflects.handleException(ex);
		}
		catch (Exception ex) {
			Reflects.handleException(ex);
		}
		
		throw new ReflectException("Invalid code path reached");
	}
	...
	protected static Object doGetVisitorAttribute() {
		return Reflects.getFieldValue((Object)null,VISITOR_ATTRIBUTES_FIELD_RECURSE);
	}

	protected static String doGetPath(Object resource) {
		return (String) Reflects.invokeMethod(VIRTUAL_FILE_METHOD_GET_PATH_NAME, resource);
	}
}