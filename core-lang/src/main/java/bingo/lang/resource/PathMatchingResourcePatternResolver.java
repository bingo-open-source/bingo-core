...
import bingo.lang.text.AntPathMatcher;
import bingo.lang.text.PathMatcher;
...
class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	private static final Log logger = LogFactory.get(PathMatchingResourcePatternResolver.class);
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			else if (rootDirResource.getURL().getProtocol().startsWith(Resources.URL_PROTOCOL_VFS)) {
	...
	protected Resource resolveRootDirResource(Resource original) throws IOException {
		if (equinoxResolveMethod != null) {
			...
			if (url.getProtocol().startsWith("bundle")) {
				return new UrlResource((URL) Reflects.invokeMethod(equinoxResolveMethod, null, url));
			}
		}
		return original;
	}
	 ...
	 * @see #doFindPathMatchingJarResources
	 * @see bingo.lang.resource.springframework.util.Resources#isJarURL
	 */
	protected boolean isJarResource(Resource resource) throws IOException {
		return Resources.isJarURL(resource.getURL());
	}
	...
	protected Set<Resource> doFindPathMatchingJarResources(Resource rootDirResource, String subPattern)
		...
		if (con instanceof JarURLConnection) {
			...
			int separatorIndex = urlFile.indexOf(Resources.JAR_URL_SEPARATOR);
			if (separatorIndex != -1) {
				jarFileUrl = urlFile.substring(0, separatorIndex);
				rootEntryPath = urlFile.substring(separatorIndex + Resources.JAR_URL_SEPARATOR.length());
	...
	protected JarFile getJarFile(String jarFileUrl) throws IOException {
		if (jarFileUrl.startsWith(Resources.FILE_URL_PREFIX)) {
			try {
				return new JarFile(Resources.toURI(jarFileUrl).getSchemeSpecificPart());
			}
			catch (URISyntaxException ex) {
				// Fallback for URLs that are not valid URIs (should hardly ever happen).
				return new JarFile(jarFileUrl.substring(Resources.FILE_URL_PREFIX.length()));
			}
		}
		else {
