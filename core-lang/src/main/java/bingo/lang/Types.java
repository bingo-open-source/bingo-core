...
public class Types {
	...
	/**
	 * <p>
	 * Gets the type arguments of a class/interface based on a subtype. For instance, this method will determine that
	 * both of the parameters for the interface {@link Map} are {@link Object} for the subtype
	 * {@link java.util.Properties Properties} even though the subtype does not directly implement the <code>Map</code>
	 * interface.
	 * <p>
	 * </p>
	 * This method returns <code>null</code> if <code>type</code> is not assignable to <code>toClass</code>. It returns
	 * an empty map if none of the classes or interfaces in its inheritance hierarchy specify any type arguments. </p>
	 * <p>
	 * A side-effect of this method is that it also retrieves the type arguments for the classes and interfaces that are
	 * part of the hierarchy between <code>type</code> and <code>toClass</code>. So with the above example, this method
	 * will also determine that the type arguments for {@link java.util.Hashtable Hashtable} are also both
	 * <code>Object</code>. In cases where the interface specified by <code>toClass</code> is (indirectly) implemented
	 * more than once (e.g. where <code>toClass</code> specifies the interface {@link java.lang.Iterable Iterable} and
	 * <code>type</code> specifies a parameterized type that implements both {@link java.util.Set Set} and
	 * {@link java.util.Collection Collection}), this method will look at the inheritance hierarchy of only one of the
	 * implementations/subclasses; the first interface encountered that isn't a subinterface to one of the others in the
	 * <code>type</code> to <code>toClass</code> hierarchy.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Return a map of the type arguments of <code>type</code> in the context of <code>toClass</code>.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Return a map of the type arguments of a parameterized type in the context of <code>toClass</code>.
	 * </p>
	 * 
	 ...
	 * @return the map with type arguments
	 */
	private static Map<TypeVariable<?>, Type> getTypeArguments(ParameterizedType parameterizedType, Class<?> toClass,
	 ...
	 * This method is the inverse of {@link #getTypeArguments(Type, Class)} which gets a class/interface's type
	 * arguments based on a subtype. It is far more limited in determining the type arguments for the subject class's
	 * type variables in that it can only determine those parameters that map from the subject {@link Class} object to
	 * the supertype.
	 * </p>
	 * <p>
	 * Example: {@link java.util.TreeSet TreeSet} sets its parameter as the parameter for {@link java.util.NavigableSet
	 * NavigableSet}, which in turn sets the parameter of {@link java.util.SortedSet}, which in turn sets the parameter
	 * of {@link Set}, which in turn sets the parameter of {@link java.util.Collection}, which in turn sets the
	 * parameter of {@link java.lang.Iterable}. Since <code>TreeSet</code>'s parameter maps (indirectly) to
	...
	/**
	 * <p>
	 * Checks if the subject type may be implicitly cast to the target parameterized type following the Java generics
	 * rules.
	 * </p>
	 * 
	 * @param type the subject type to be assigned to the target type
	...
	/**
	 * <p>
	 * Checks if the subject type may be implicitly cast to the target generic array type following the Java generics
	 * rules.
	 * </p>
	 * 
	 * @param type the subject type to be assigned to the target type
	...
	/**
	 * <p>
	 * Returns an array containing the sole type of {@link Object} if {@link TypeVariable#getBounds()} returns an empty
	 * array. Otherwise, it returns the result of <code>TypeVariable.getBounds()</code> passed into
	...
	/**
	 * <p>
	 * Returns an array containing the sole value of {@link Object} if {@link WildcardType#getUpperBounds()} returns an
	 * empty array. Otherwise, it returns the result of <code>WildcardType.getUpperBounds()</code> passed into
	 * {@link #normalizeUpperBounds}.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Returns an array containing a single value of <code>null</code> if {@link WildcardType#getLowerBounds()} returns
	 * an empty array. Otherwise, it returns the result of <code>WildcardType.getLowerBounds()</code>.
	 * </p>
	 * 
	 * @param wildcardType the subject wildcard type
	...
	/**
	 * <p>
	 * This method strips out the redundant upper bound types in type variable types and wildcard types (or it would
	 * with wildcard types if multiple upper bounds were allowed).
	 * </p>
	 * <p>
	 ...
	 * </pre>
	 * 
	 * since <code>List</code> is a subinterface of <code>Collection</code>, this method will return the bounds as if
	 ...
	 * </p>
	 * 
	 * @param bounds an array of types representing the upper bounds of either <code>WildcardType</code> or
