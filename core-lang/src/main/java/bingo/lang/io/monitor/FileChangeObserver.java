 ...
 * <ul>
 *   <li>Create {@link FileChangeListener} implementation(s) that process
 *      the file/directory create, change and delete events</li>
 *   <li>Register the listener(s) with a {@link FileChangeObserver} for
 *       the appropriate directory.</li>
 *   <li>Either register the observer(s) with a {@link FileChangeMonitor} or
 *       run manually.</li>
 * </ul>
 *
 * <h2>Basic Usage</h2>
...
public class FileChangeObserver implements Serializable {
    ...
    protected FileChangeObserver(FileInfo rootEntry, FileFilter fileFilter, IOCase caseSensitivity) {
        ...
        if (caseSensitivity == null || caseSensitivity.equals(IOCase.SYSTEM)) {
            this.comparator = Comparators.nameSystem();
        } else if (caseSensitivity.equals(IOCase.INSENSITIVE)) {
            this.comparator = Comparators.nameInsensitive();
        } else {
            this.comparator = Comparators.nameSensitive();
        }
    }
