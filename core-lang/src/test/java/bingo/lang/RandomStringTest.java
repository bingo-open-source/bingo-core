...
/**
 * Unit tests {@link org.apache.commons.Randoms.RandomStringUtils}.
 *
 * @version $Id: RandomStringUtilsTest.java 1199894 2011-11-09 17:53:59Z ggregory $
 */
public class RandomStringTest extends junit.framework.TestCase {
    /**
     * Construct a new instance of RandomStringUtilsTest with the specified name
     */
    public RandomStringTest(String name) {
        super(name);
    }

    //-----------------------------------------------------------------------
    public void testConstructor() {
        assertNotNull(new Randoms());
        Constructor<?>[] cons = Randoms.class.getDeclaredConstructors();
        assertEquals(1, cons.length);
        assertEquals(true, Modifier.isPublic(cons[0].getModifiers()));
        assertEquals(true, Modifier.isPublic(Randoms.class.getModifiers()));
        assertEquals(false, Modifier.isFinal(Randoms.class.getModifiers()));
    }
    ...
    public void testRandomStringUtils() {
        ...
        r2 = Randoms.nextStringAlphabetic(50);
        assertTrue("!r1.equals(r2)", !r1.equals(r2));
        
        r1 = Randoms.nextStringAlphanumeric(50);
        ...
        r1 = Randoms.nextStringNumeric(50);
        ...
        r2 = Randoms.nextStringNumeric(50);
        assertTrue("!r1.equals(r2)", !r1.equals(r2));
        
        String set = "abcdefg";
        r1 = Randoms.nextString(50, set);
        assertEquals("random(50, \"abcdefg\")", 50, r1.length());
        for(int i = 0; i < r1.length(); i++) {
            assertTrue("random char in set", set.indexOf(r1.charAt(i)) > -1);
        }
        r2 = Randoms.nextString(50, set);
        assertTrue("!r1.equals(r2)", !r1.equals(r2));
        
        r1 = Randoms.nextString(50, (String) null);
        assertEquals("random(50) length", 50, r1.length());
        r2 = Randoms.nextString(50, (String) null);
        assertEquals("random(50) length", 50, r2.length());
        assertTrue("!r1.equals(r2)", !r1.equals(r2));
        
        set = "stuvwxyz";
        r1 = Randoms.nextString(50, set.toCharArray());
        ...
        r1 = Randoms.nextString(50, (char[]) null);
        assertEquals("random(50) length", 50, r1.length());
        r2 = Randoms.nextString(50, (char[]) null);
        assertEquals("random(50) length", 50, r2.length());
        assertTrue("!r1.equals(r2)", !r1.equals(r2));

        long seed = System.currentTimeMillis();
        r1 = Randoms.nextString(50,0,0,true,true,null,new Random(seed));
        r2 = Randoms.nextString(50,0,0,true,true,null,new Random(seed));
        assertEquals("r1.equals(r2)", r1, r2);

        r1 = Randoms.nextString(0);
        assertEquals("random(0).equals(\"\")", "", r1);

    }
    public void testExceptions() {
        try {
            Randoms.nextString(-1);
            fail();
        } catch (IllegalArgumentException ex) {}
        try {
            Randoms.nextString(-1, true, true);
            fail();
        } catch (IllegalArgumentException ex) {}
        try {
            Randoms.nextString(-1, new char[0]);
            fail();
        } catch (IllegalArgumentException ex) {}
        try {
            Randoms.nextString(-1, "");
        ...
        try {
            Randoms.nextString(-1, 'a', 'z', false, false);
            fail();
        } catch (IllegalArgumentException ex) {}
        try {
            Randoms.nextString(-1, 'a', 'z', false, false, new char[0]);
            fail();
        } catch (IllegalArgumentException ex) {}
        try {
            Randoms.nextString(-1, 'a', 'z', false, false, new char[0], new Random());
            fail();
        } catch (IllegalArgumentException ex) {}
    }
    ...
    public void testRandomAlphaNumeric() {
        ...
        for (int i = 0; i < 100; i++) {
            String randString = Randoms.nextStringAlphanumeric(10);
    ...
    public void testRandomNumeric() {
        ...
        for (int i = 0; i < 100; i++) {
            String randString = Randoms.nextStringNumeric(10);
            for (int j = 0; j < testChars.length; j++) {
                if (randString.indexOf(testChars[j]) > 0) {
                    found[j] = true;
                }
    ...
    public void testRandomAlphabetic() {
        ...
        for (int i = 0; i < 100; i++) {
            String randString = Randoms.nextStringAlphabetic(10);
            for (int j = 0; j < testChars.length; j++) {
                if (randString.indexOf(testChars[j]) > 0) {
                    found[j] = true;
                }
    ...
    /**
     * Make sure 33 and 127 are generated by randomNumeric
     * This test will fail randomly with probability = 2*(95/96)**1000 ~ 5.7E-5
     */  
    public void testRandomAscii() {
        char[] testChars = {(char) 33, (char) 126};
        boolean[] found = {false, false};
        for (int i = 0; i < 100; i++) {
            String randString = Randoms.nextString(10);
            for (int j = 0; j < testChars.length; j++) {
                if (randString.indexOf(testChars[j]) > 0) {
                    found[j] = true;
                }
    ...
    public void testRandomStringUtilsHomog() {
        ...
        for (int i = 0; i< 100; i++) {
           gen = Randoms.nextString(6,chars);
           for (int j = 0; j < 6; j++) {
               switch (gen.charAt(j)) {
    ...
    /**
     * Checks if the string got by {@link Randoms#nextString(int)}
    ...
    public void testLang100() throws Exception {
        ...
        String orig = Randoms.nextString(size);
