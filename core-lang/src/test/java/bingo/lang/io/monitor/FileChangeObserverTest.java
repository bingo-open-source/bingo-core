...
public class FileChangeObserverTest extends AbstractMonitorTestCase {
     ...
     */
    public FileChangeObserverTest(String name) {
        super(name);
    ...
    public void testAddRemoveListeners() {
        ...
        // Add Listener
        FileChangeListenerAdaptor listener = new FileChangeListenerAdaptor();
        observer.addListener(listener);
        Iterator<FileChangeListener> it = observer.getListeners().iterator();
    ...
    public void testToString() {
        ...
        FileChangeObserver observer = null;

        observer = new FileChangeObserver(file);
        assertEquals("FileChangeObserver[file='" + file.getPath() +  "', listeners=0]",
                observer.toString());
  
        observer = new FileChangeObserver(file, Filters.canRead());
        assertEquals("FileChangeObserver[file='" + file.getPath() +  "', CanReadFilter, listeners=0]",
                observer.toString());
    ...
    public void testDirectory() {
        try {
            ...
            Files.deleteDirectory(testDirB);
            checkAndNotify();
    ...
    public void testFileDelete() {
        try {
            ...
            // Delete first entry
            Files.deleteQuietly(testDirAFile1);
            testDirA = touch(testDirA);
            checkAndNotify();
            ...
            Files.deleteQuietly(testDirAFile3);
            testDirA = touch(testDirA);
            checkAndNotify();
            ...
            // Delete last entry
            Files.deleteQuietly(testDirAFile5);
            testDirA = touch(testDirA);
            checkAndNotify();
    ...
    public void testObserveSingleFile() {
        try {
            ...
            FileFilter nameFilter = Filters.nameEquals(testDirAFile1.getName());
            ...
            // Delete
            Files.deleteQuietly(testDirAFile1);
            Files.deleteQuietly(testDirAFile2);
            Files.deleteQuietly(testDirAFile3);
    ...
    /**
     * Call {@link FileChangeObserver#checkAndNotify()}.
     *
