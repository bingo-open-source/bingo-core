...
import bingo.lang.io.Filter;
import bingo.lang.io.Filters;

/**
 * {@link FileChangeObserver} Test Case.
 */
abstract class AbstractMonitorTestCase extends TestCase {

    /** File observer */
    protected FileChangeObserver observer;
    ...
    protected void setUp() throws Exception {
        ...
        if (testDir.exists()) {
            Files.cleanDirectory(testDir);
        } else {
            testDir.mkdir();
        }

        Filter files 	   = Filters.isFile();
        Filter javaSuffix  = Filters.endsWith(".java");
        Filter fileFilter  = Filters.and(files, javaSuffix);
        Filter directories = Filters.isDirectory();
        Filter visible 	   = Filters.isVisible();
        Filter dirFilter   = Filters.and(directories, visible);
        Filter filter 	   = Filters.or(dirFilter, fileFilter);
        
        createObserver(testDir, filter);
    }

    /**
     * Create a {@link FileChangeObserver}.
     * 
    ...
    protected void createObserver(File file, FileFilter fileFilter) {
        observer = new FileChangeObserver(file, fileFilter);
